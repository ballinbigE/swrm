...
export function syncMarkdownToSqlite(
  ...
  mdFiles: string[],
  opts: { boardSlug?: string; boardId?: number } = {},
): SyncResult {
  ...
  // If boardId was supplied in opts, use it; otherwise fall back to what we
  // looked up by slug (the two should match, but opts.boardId is the
  // authoritative scoping key for the archive query).
  const scopedBoardId: number | undefined = opts.boardId ?? undefined;
  ...
  // When a boardId scope is provided, restrict the archive to that board so
  // syncing one project's markdown cannot archive another project's tasks.
  const archiveStmt = scopedBoardId !== undefined
    ? db.prepare(
  ...
  const archResult = scopedBoardId !== undefined
    ? archiveStmt.run(JSON.stringify(Array.from(seenRefs)), scopedBoardId)
    : archiveStmt.run(JSON.stringify(Array.from(seenRefs)));
...
/**
 * Sync the markdown task files for a single project into its first board.
 * Reads `<root_path>/tasks/todo.md` and `<root_path>/tasks/backlog.md`.
 * Returns a zero SyncResult (no-op) if the project has no boards yet.
 */
export function syncProjectMarkdown(
  db: Database.Database,
  project: { slug: string; root_path: string },
): SyncResult {
  const board = db
    .prepare(
      `SELECT id, slug FROM boards
       WHERE project_id = (SELECT id FROM projects WHERE slug = ?)
       ORDER BY position, id LIMIT 1`,
    )
    .get(project.slug) as { id: number; slug: string } | undefined;

  if (!board) {
    return { parsed: 0, inserted: 0, archived: 0, unchanged: 0 };
  }

  const files = [
    path.join(project.root_path, 'tasks', 'todo.md'),
    path.join(project.root_path, 'tasks', 'backlog.md'),
  ];

  return syncMarkdownToSqlite(db, files, { boardSlug: board.slug, boardId: board.id });
}
